...
func StoreTime(db *bbolt.DB, index int64, transaction *pb.Transaction, timeTaken time.Duration) error {
	// Open the bucket or create it if it doesn't exist
	err := db.Update(func(txn *bbolt.Tx) error {
		bucket, err := txn.CreateBucketIfNotExists([]byte("txn"))
		if err != nil {
...
func DeleteRecord(db *bbolt.DB, tableName string, key string) error {
	err := db.Update(func(txn *bbolt.Tx) error {
		bucket := txn.Bucket([]byte(tableName))
		if bucket == nil {
			return fmt.Errorf("bucket %s not found", tableName)
...
func DeleteAllRecords(db *bbolt.DB, tableName string) error {
	err := db.Update(func(txn *bbolt.Tx) error {
		bucket := txn.Bucket([]byte(tableName))
		if bucket == nil {
			return fmt.Errorf("bucket %s not found", tableName)
		...
		// Iterate over the bucket and delete all records
		return bucket.ForEach(func(k, v []byte) error {
			return bucket.Delete(k) // Delete each record
		})
